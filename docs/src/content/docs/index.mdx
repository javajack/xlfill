...
<CardGrid>
	<Card title="Streaming mode" icon="rocket">
		`WithStreaming(true)` writes output incrementally — 3x faster and 60% less memory for large reports. Generate 100K-row workbooks without breaking a sweat.
	</Card>
	<Card title="Parallel processing" icon="setting">
		`WithParallelism(4)` runs loop iterations concurrently. Fixed-height areas are parallelized automatically; variable-height falls back safely.
	</Card>
	<Card title="Structured errors" icon="warning">
		Every error tells you *what* went wrong (Template / Data / Runtime), *where* (cell reference), and *how to fix it*. Unknown commands get "did you mean?" suggestions.
	</Card>
	<Card title="Auto-mode" icon="star">
		`WithAutoMode(hint)` analyzes your template and picks the optimal strategy — streaming, parallel, or sequential. Zero config, maximum throughput.
	</Card>
</CardGrid>
