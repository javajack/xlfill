...
- **`condition` must return a bool.** `condition="e.Name"` errors out — `Name` is a string. Use `condition="e.Name != ''"` instead.
- **`nil` is treated as false** (matches JXLS v3 behavior). So `condition="e.Manager"` quietly works even when `Manager` might be unset, but it won't error if your field name is wrong — be careful.
- **`select` on `jx:each` is often what you want instead.** If you're filtering rows in a loop, `jx:each(... select="e.Active")` is cleaner than `jx:each + jx:if` inside.
- **Else-area is opt-in.** Without `elseArea`, a false condition just leaves a gap (rows below shift up). If you want a "no results" message, define both ifArea and elseArea.
